...
Replaces the Celery task chain (fetch_news_content -> deep_filter/evaluate -> embed)
with a single LangGraph workflow executed by the process_news_article Celery task.

Workflow graph:
    START -> fetch_content -> route_filter_mode
...
async def fetch_content_node(state: NewsProcessingState) -> dict:
    """Fetch full article content and save to file storage."""
    ...
    if skill is None:
        return {
            "fetch_error": "fetch_full_content skill not found in registry",
            "final_status": "failed",
            "error": "fetch_full_content skill not found in registry",
        }
    ...
    result = await skill.safe_execute(
        ...
        polygon_api_key=state.get("polygon_api_key"),
    ...
    if not result.success:
        ...
        try:
            ...
            logger.warning("fetch_content_node: failed to update DB status for %s: %s", news_id, e)
    ...
    try:
        async with get_task_session() as db:
            ...
            if news:
                ...
                await db.commit()
                logger.info(
                    "fetch_content_node: news_id=%s, %d words, status=%s",
                    news_id,
                    data.get("word_count", 0),
                    content_status,
                )
    except Exception as e:
        logger.warning("fetch_content_node: failed to update DB for %s: %s", news_id, e)
...
async def deep_filter_node(state: NewsProcessingState) -> dict:
    ...
    title = state.get("title", "")
    source = ""  # Will be read from DB
    url = state["url"]

    # Get source from DB and execute deep filter in a single session
    async with get_task_session() as db:
        from sqlalchemy import select
        from app.models.news import News

        query = select(News.source).where(News.id == uuid.UUID(state["news_id"]))
        res = await db.execute(query)
        row = res.first()
        if row:
            source = row[0] or ""
...
async def single_filter_node(state: NewsProcessingState) -> dict:
    ...
    return {"filter_decision": "keep" if should_keep else "delete"}
    ...
    async with get_task_session() as db:
        ...
        if final_status == "embedded":
            ...
            if news.content_status not in (
                ...
                # Only update if not already set by fetch_content_node
                if "embed" in error.lower():
                    news.content_status = ContentStatus.EMBEDDING_FAILED.value
                # else: keep whatever fetch_content_node set
...
def create_news_pipeline() -> StateGraph:
    ...
    # START -> fetch_content
    workflow.add_edge("__start__", "fetch_content")

    # fetch_content -> route by filter mode
    workflow.add_conditional_edges(
        ...
        route_filter_mode,
        {
            ...
            "end": "update_db",  # Skip filter if fetch failed
        },
    )
