...
class AnthropicProvider(LLMProvider):
    ...
    def _build_api_kwargs(self, request: ChatRequest) -> Dict[str, Any]:
        ...
        Passes temperature when explicitly set.  max_tokens is always
        passed (required by Anthropic API, defaults to 4096).
        Propagates timeout to prevent SDK's 600s default from causing
        orphaned connections when the backend httpx client times out first.
        """
        ...
        # Temperature — pass through when explicitly set
        if request.temperature is not None:
            kwargs["temperature"] = request.temperature

        # Timeout — propagate to Anthropic SDK to prevent 600s default
        # which outlives the backend's httpx timeout and causes orphaned connections
        if request.timeout:
